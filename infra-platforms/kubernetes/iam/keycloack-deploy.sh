...
kubectl apply -f keycloack-ns.yaml

echo -e "${YELLOW}Second Step: Install Helm Chart Overriding values${NC}"

CLUSTER_IP=$(kubectl describe svc postgresql -n postgresql16-ns | grep "IP:" | awk '{print $2}')

helm upgrade --install keycloak --values values.yaml --set externalDatabase.host=$CLUSTER_IP bitnami/keycloak --namespace keycloack-ns
